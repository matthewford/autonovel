...
from llm import ensure_llm_configured, generate, model_fingerprint
...
def call_opus(prompt, max_tokens=8000):
    ...
    print(f"Sending to {model_fingerprint('reviewer')} ({len(prompt):,} chars)...", file=sys.stderr)
    return generate(
        prompt,
        role="reviewer",
        ...
        temperature=0.3,
        timeout=600,
        system="You are a rigorous literary reviewer and fiction professor. Be specific, fair, and actionable.",
    )


def get_title():
    """Extract novel title from first chapter or outline."""
    configured = get_novel_title(BASE_DIR)
    if configured != "the novel":
        return configured
...
def main():
    ...
    ensure_llm_configured("reviewer")
